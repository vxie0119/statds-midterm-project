...
invalid_zip_codes = clean[~clean['ZIP_CODE'].apply(lambda x: str(x).isdigit() 
                                                   and 10000 <= int(x) <= 11697) 
                                                   & (data['ZIP_CODE'] != 0)]

# Identifying invalid boroughs
known_boroughs = ["Manhattan", "Brooklyn", "Queens", "Bronx", "Staten Island"]
invalid_boroughs = clean[~clean['BOROUGH'].isin(known_boroughs) 
                                           & data['BOROUGH'].notnull()]
...
n_initpass = initial_inspections[initial_inspections['passing'] == 1].groupby('ZIP_CODE')
                                ['JOB_TICKET_OR_WORK_ORDER_ID'].count().rename('n_initpass')
...
n_comppass = compliance_inspections[compliance_inspections['passing'] == 1].groupby('ZIP_CODE')
                                   ['JOB_TICKET_OR_WORK_ORDER_ID'].count().rename('n_comppass')

# Combining the results into one dataframe
aggregated_data = pd.concat([n_initial, n_initpass, n_compliance, n_comppass], axis=1)
...
aggregated_sightings['n_sighting'] = aggregated_sightings['n_sighting'].fillna(0).astype(int)
...
aggregated_sightings['initial_pass_rate'].fillna(0, inplace=True)
aggregated_sightings.replace([float('inf'), -float('inf')], 0, inplace=True)
...
X = aggregated_sightings[['n_sighting']] # Predictor variables
X = sm.add_constant(X) # Adding a constant term for the regression
y = aggregated_sightings['initial_pass_rate'] # Response variable
...
- How does the density of rodent sightings (as reported through 311 service requests) correlate with the success rate of initial rodent inspections in different New York City zip codes?
- Has the number of rodent sightings and the pass rate of inspections changed over time (monthly or yearly)?
- Which areas (zip codes) in New York City have the highest frequency of rodent sightings and the lowest inspection pass rates?
...
- Do areas with high initial inspection failure rates show improvement in compliance inspections?
- Is there a seasonal pattern to rodent sightings and inspection outcomes?
- 
