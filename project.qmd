...
invalid_zip_codes = clean[~clean['ZIP_CODE'].apply(lambda x: str(x).isdigit() and 10000 <= int(x) <= 11697) & (data['ZIP_CODE'] != 0)]

# Identifying invalid boroughs
known_boroughs = ["Manhattan", "Brooklyn", "Queens", "Bronx", "Staten Island"]
invalid_boroughs = clean[~clean['BOROUGH'].isin(known_boroughs) & data['BOROUGH'].notnull()]
...
initial_contingency_table = pd.crosstab(initial_inspections['BOROUGH'], initial_inspections['passing'])
chi2_initial, p_initial, dof_initial, expected_initial = chi2_contingency(initial_contingency_table)

# Chi-Squared Test for Compliance Inspections
compliance_inspections = clean[clean['INSPECTION_TYPE'] == 'Compliance']
compliance_contingency_table = pd.crosstab(compliance_inspections['BOROUGH'], compliance_inspections['passing'])

chi2_compliance, p_compliance, dof_compliance, expected_compliance = chi2_contingency(compliance_contingency_table)
...
print(initial_contingency_table)
print(chi2_initial, p_initial)

print('Compliance Inspections:')
print(compliance_contingency_table)
print(chi2_compliance, p_compliance)
```
...
clean['TIME_OF_DAY'] = clean['INSPECTION_DATE'].apply(lambda x: 'Morning' if x.hour < 12 else 'Afternoon')

# Define your criteria for 'passing' and categorize the 'passing' column
clean['passing'] = clean['RESULT'].apply(lambda x: 1 if x == 'Passed' else 0)

# Calculate the passing rate by borough, inspection type, and time of day
passing_rate_comparison = clean.groupby(['BOROUGH', 'INSPECTION_TYPE', 'TIME_OF_DAY'])['passing'].mean().reset_index()
...
sns.barplot(data=passing_rate_comparison, x='BOROUGH', y='passing', hue='TIME_OF_DAY', ci=None)
...
clean['TIME_OF_DAY'] = clean['INSPECTION_DATE'].dt.hour.apply(lambda x: 'Morning' if x < 12 else 'Afternoon')
...
n_initial = initial_inspections.groupby('ZIP_CODE')['JOB_TICKET_OR_WORK_ORDER_ID'].count().rename('n_initial')
n_initpass = initial_inspections[initial_inspections['passing'] == 1].groupby('ZIP_CODE')['JOB_TICKET_OR_WORK_ORDER_ID'].count().rename('n_initpass')

# Aggregating data for compliance inspections
n_compliance = compliance_inspections.groupby('ZIP_CODE')['JOB_TICKET_OR_WORK_ORDER_ID'].count().rename('n_compliance')
n_comppass = compliance_inspections[compliance_inspections['passing'] == 1].groupby('ZIP_CODE')['JOB_TICKET_OR_WORK_ORDER_ID'].count().rename('n_comppass')

# Combining the results into one dataframe
aggregated_data = pd.concat([n_initial, n_initpass, n_compliance, n_comppass], axis=1).fillna(0).reset_index()
...
aggregated_data_with_sightings = pd.merge(aggregated_data, n_sighting, how='left', left_on='ZIP_CODE', right_index=True)

# Fill NaN values with 0 for zip codes with no recorded sightings
aggregated_data_with_sightings['n_sighting'] = aggregated_data_with_sightings['n_sighting'].fillna(0).astype(int)

aggregated_data_with_sightings.head()
```
...
zipcode_data = [fetch_zipcode_data(zip_code) for zip_code in aggregated_data_with_sightings['ZIP_CODE'].unique()]
...
aggregated_data_with_sightings['initial_pass_rate'] = aggregated_data_with_sightings['n_initpass'] / aggregated_data_with_sightings['n_initial']

# Replace NaN values and infinite values (due to division by zero) with zeros
aggregated_data_with_sightings['initial_pass_rate'].fillna(0, inplace=True)
aggregated_data_with_sightings.replace([float('inf'), -float('inf')], 0, inplace=True)

# Binomial regression model
# Assuming we want to include 'n_sighting' as a predictor for the initial pass rate.
# Additional variables can be added as needed based on the available data.
X = aggregated_data_with_sightings[['n_sighting']] # Predictor variables
X = sm.add_constant(X) # Adding a constant term for the regression
y = aggregated_data_with_sightings['initial_pass_rate'] # Response variable
...
```{python}
# Plotting the binomial regression model
plt.scatter(X['n_sighting'], y, alpha=0.5, label='Data Points')
plt.xlabel('Number of Sightings (n_sighting)')
plt.ylabel('Initial Pass Rate')
plt.title('Binomial Regression Model for Initial Inspection Pass Rate')

# Generating predictions for the plot
x_pred = np.linspace(X['n_sighting'].min(), X['n_sighting'].max(), 100)
x_pred_with_const = sm.add_constant(x_pred)
y_pred = binom_results.predict(x_pred_with_const)

plt.plot(x_pred, y_pred, color='red', label='Fitted Model')
plt.legend()
plt.show()
```
##  Research Question
